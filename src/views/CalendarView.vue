...
import zhCn from 'vue-cal/dist/i18n/zh-cn.es.js'
import dayjs from 'dayjs'
import { computed, onMounted, ref } from 'vue'
import BaseCard from '@/components/base/BaseCard.vue'
import BaseButton from '@/components/base/BaseButton.vue'
import { useEventStore } from '@/stores/eventStore'
import { mapEventsToDateIndicators } from '@/services/eventCalendarMapper'
...
/** vue-cal 3.x 需要 Date 对象做 active/selected 日期 */
const activeDate = computed(() => viewDate.value)
    ...
    <BaseCard padding="lg" class="calendar-wrap">
      ...
      <div class="mymemo-cal">
        <VueCal
          ...
          hide-side-bar
          :disable-events-drag="true"
          :disable-events-overlap="false"
          @cell-click="onCellClick"
        >
          <!-- 单元格渲染：日期号 + 轻量事件圆点 indicator -->
          <template #cell="cell">
            <div class="vc-cell-inner">
              <span
                class="vc-day-num"
                :class="{
                  'is-today': fmtKey(cell.date) === todayKey,
                  'is-weekday-outside': !cell.currentMonth,
                }"
              >
                {{ dayjs(cell.date).date() }}
              </span>

              <div class="vc-indicators" v-if="indicators[fmtKey(cell.date)]?.hasEvent">
                <span
                  v-for="t in orderedTypes(fmtKey(cell.date)).slice(0, 3)"
                  :key="t"
                  class="vc-dot"
...
.mymemo-cal :deep(.vuecal__weekdays-headings .vuecal__weekday-heading) {
  border: none;
  background: transparent;
  color: inherit;
  font-size: inherit;
...
.mymemo-cal :deep(.vuecal__cell) {
  ...
  border-right-color: var(--surface-border-soft);
  border-bottom-color: var(--surface-border-soft);
  transition: background var(--duration-fast) var(--ease-out),
    border-color var(--duration-fast) var(--ease-out);
}
.mymemo-cal :deep(.vuecal__cell:last-child) {
  border-right-color: transparent;
}
.mymemo-cal :deep(.vuecal__week:last-child .vuecal__cell) {
  border-bottom-color: transparent;
}
.mymemo-cal :deep(.vuecal__cell.vuecal--selected) {
  background: var(--accent-soft);
  border-color: var(--color-primary-light);
  border-radius: 10px;
  box-shadow: var(--highlight-glow);
}
.mymemo-cal :deep(.vuecal__cell:not(.vuecal--out-of-month):hover) {
  background: var(--glass-bg-hover);
  border-radius: 10px;
}
.mymemo-cal :deep(.vuecal__cell.vuecal--out-of-month) {
  background: transparent;
  opacity: 1;
}
